(function(){
  ...
  if(isPM && !woExisting){
    ...
    genBtn.addEventListener("click", ()=>{
      ...
      if(!proj) proj = DB.projects.create({ tenderId:tender.id, masterProjectId:tender.masterProjectId||null, workOrderId:wo.id, name:tender.title, pmId:pm.id, contractorId:contractor.id, district:tender.district, state:tender.state, startDate:tender.startDate, endDate:tender.endDate, status:"running", progressPct:0 });
