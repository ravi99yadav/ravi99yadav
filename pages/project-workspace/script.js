(function(){
  ...
  if(!project){
    ...
    document.getElementById("pickerGrid").innerHTML = list.length ? list.map(p=>`
      <div class="card card-hover">
        <b>${U.escapeHtml(p.name)}</b><div class="text-muted" style="font-size:12px">${p.district}, ${p.state}</div>
      ...
      </div>`).join("") : `<div class="empty-state" style="grid-column:1/-1"><div class="es-icon">🏗️</div>No projects yet — a project is created automatically once a Work Order is issued.</div>`;
    return;
  }
  ...
  function renderHeader(){
    document.getElementById("projHeader").innerHTML = `
      <div class="flex justify-between items-start" style="flex-wrap:wrap;gap:12px">
        <div><h1 style="margin:0">${U.escapeHtml(project.name)}</h1><p class="text-muted">${project.district}, ${project.state} · ${isPM?'Contractor':'Project Manager'}: ${U.escapeHtml(otherUser?otherUser.name:"—")} · ${U.fmtDate(project.startDate)} – ${U.fmtDate(project.endDate)}</p></div>
  ...
  function renderRABill(){
    ...
    document.getElementById("panelRABill").innerHTML = `
      <div class="flex justify-between mb-3"><h3>RA Bills</h3>${!isPM?'<button class="btn btn-primary btn-sm" id="newRABtn">+ Raise RA Bill</button>':''}</div>
      ...
      <tbody>${bills.length ? bills.map(b=>{
        ...
        <td>${isPM && b.status==='pending' ? `<button class="btn btn-sm btn-success" data-approve-bill="${b.id}">Approve</button> <button class="btn btn-sm btn-danger" data-reject-bill="${b.id}">Reject</button>` : `<button class="btn btn-sm btn-ghost" data-print-bill="${b.id}">Print</button>`}</td></tr>`;
    ...
    document.getElementById("panelRABill").addEventListener("click", e=>{
      const app = e.target.closest("[data-approve-bill]"); const rej = e.target.closest("[data-reject-bill]"); const print = e.target.closest("[data-print-bill]");
  ...
  function openRABillModal(){
    ...
    document.getElementById("genericModalBody").innerHTML = `
      ...
      <div class="field"><label>Claim basis</label>
        <div class="pill-tab"><button class="active" data-basis="pct">% of BOQ item</button><button data-basis="manual">Manual amount</button></div>
        <p class="hint">Use the MB Sheet abstract to compute measured quantities, then either enter a manual claim amount above or note the % against last billed value here.</p>
      </div>
      <div class="input-group">
    ...
    document.getElementById("rbSave").addEventListener("click", ()=>{
      const gross = +document.getElementById("rbGross").value||0;
  ...
  function renderDPR(){
    ...
    document.getElementById("panelDPR").innerHTML = `
      <div class="flex justify-between mb-3"><h3>Daily Progress Reports</h3>${!isPM?'<button class="btn btn-primary btn-sm" id="newDPRBtn">+ Log Today\'s DPR</button>':''}</div>
  ...
  function renderHindrance(){
    ...
    document.getElementById("panelHindrance").innerHTML = `
      <div class="flex justify-between mb-3"><h3>Hindrance Register</h3>${!isPM?'<button class="btn btn-primary btn-sm" id="newHindBtn">+ Raise Hindrance</button>':''}</div>
  ...
  function renderPayments(){
    ...
    document.getElementById("panelPayments").innerHTML = `
      <div class="flex justify-between mb-3"><h3>Payment Requests</h3>${!isPM?'<button class="btn btn-primary btn-sm" id="newPayBtn">+ Request Payment</button>':''}</div>
  ...
  SW.UI.helpSection(document.querySelector(".app-content"), "Project Workspace", [
    ...
    "RA Bill: contractors raise bills referencing the previous approved bill; retention, advance recovery, GST and TDS are calculated automatically. Project Managers approve or reject.",
